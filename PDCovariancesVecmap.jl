...
function replaceEigens(covM)
    E = covM |> Array |> eigen;
    E.values[E.values .< 1] .= 1;
    newCov = E.vectors * diagm(E.values) * E.vectors';
    newCov = (newCov * newCov') ./ 2
    return newCov
end

covariance(E) = permutedims(E) * E |> replaceEigens;

function checkPositiveness(E)
    sym = E |> Array |> issymmetric
    pd  = E |> Array |> isposdef
    return sym, pd
end


function buildPDSeedDictionary(X::T, Y::T; sim_size::Int64=4000) where {T}
   # sims = map(cudaCorrelationMatrix, [X, Y])
    xsim = covariance(X[:, 1:sim_size]);
    println(checkPositiveness(xsim))
    ysim = covariance(Y[:, 1:sim_size]);
    println(checkPositiveness(ysim));
    sort!(ysim, dims=1)
    sort!(xsim, dims=1)
    # map(sim -> sort!(sim, dims=1), sims);
    xsim, ysim = map(normalizeEmbedding, [xsim, ysim])
    sim = (xsim' * ysim) |> relu |> normalizeEmbedding; # actually this is still the cosine similarity from X -> Z.
    checkPositiveness(sim) |> println
...
src_idx, trg_idx = buildPDSeedDictionary(subX, subY);
